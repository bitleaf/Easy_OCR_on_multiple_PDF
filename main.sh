...
if [[ $(docker --version) == *"build"* ]]; then
  ...
  echo -e "\e[31m Dependency not OK: Docker is not installed. Install from https://docs.docker.com/engine/installation/\e[0m"
  exit 1
fi

echo -e "Checking dependency 2/2: Docker image for OCR downloaded?\c"
if [[ $(docker pull jbarlow83/ocrmypdf) == *'Image is up to date'* ]]; then
  ...
  echo -e "\e[32m Image has been successfully downloaded. \e[0m" && printf "\n"
fi

## Processing OCR
for i in ./put_pdf-files_here/*; do
